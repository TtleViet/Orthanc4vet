 ...
 * @Date 03/09/2015 (DD/MM/YY)
 * @Brief : Ajoute au PACS Othanc des fonctionnalités de recherche
 * d'images en fonction de tags DICOM vétérinaires
 * (Patient
 * ResponsiblePerson 
 ...
 **/
/*Otc : That word is going to mean that code part 
 * is from the basic plugin Orthanc version.
 * And that i wouldn't like to use. 
 */
...
ORTHANC_PLUGINS_API int32_t OrthancPluginInitialize(OrthancPluginContext* c)
{
  ...
  context = c;
  OrthancPluginLogWarning(context, "4Vet plugin is initializing");
  ...
  OrthancPluginExtendOrthancExplorer(context, "alert('Hello 4Vet!  Et je retest');");
